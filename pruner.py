...
class Pruner(nn.Module):
    ...
    def __init__(self, config, model, tokenizer,
        ...
        self.bilinear = nn.Linear(emb_size * block_size, 1)
    ...
    def forward_prune(self, hs, ts, rs):
        '''
    ...
    def forward(self,
        ...
        logits = self.forward_prune(hs, ts, rs)
        # output['logits'] = logits
        # output["rel_pred"] = self.loss_fnt.get_label(logits, num_labels=self.num_labels)
        logits = logits.squeeze(-1)

        if tag in ["test", "dev"]:  # testing
            labels_ident = torch.where(labels[:, 1:].sum(-1) > 0, 1, 0)
            preds = torch.zeros_like(labels_ident)
            preds_ents = []
            total_pre_ents = 0
            for doc_id in range(len(hts)):
                ents = hts[doc_id]
                num_ents = len(ents)
                doc_logits = logits[total_pre_ents:(total_pre_ents+num_ents)]
                _, idx = torch.topk(doc_logits, int(0.5 * num_ents))
                preds[idx+total_pre_ents] = 1
                # doc_pred = [hts[doc_id][i] for i in idx]
                preds_ents.append(idx)
                total_pre_ents += num_ents

            preds_num = preds.sum()
            correct_num = (preds * labels_ident).sum()
            gold_num = labels_ident.sum()

            output["ident_statistic"] = {"preds_num": preds_num, "correct_num": correct_num, "gold_num": gold_num}
            output["rel_pred"] = preds_ents
            # preds = torch.zeros_like(labels_ident)
            # _, idx = torch.topk(logits, int(0.1 * logits.shape[0]))
            # preds[idx] = 1
            # preds_num = preds.sum()
            # correct_num = (preds * labels_ident).sum()
            # gold_num = labels_ident.sum()
            ...
            # doc_indicate = 0
            # border = 0
            # hts_cat = list(itertools.chain.from_iterable(hts))
            # idx = sorted(idx)
            #
            # while doc_indicate < len(hts):
            #     pred_doc_idx = []
            #     border += len(hts[doc_indicate])
            #     while idx_indicate < len(idx):
            #         sel_idx = idx[idx_indicate]
            #         if sel_idx < border:
            #             pred_doc_idx.append(hts_cat[sel_idx])
            #             idx_indicate += 1
            #         else:
            #             break
            #     pred_batch_idx.append(pred_doc_idx)
            #     doc_indicate += 1
            # breakpoint()
            # output["rel_pred"] = pred_batch_idx

        else:  # training
            # relation extraction loss
            labels_ident = torch.where(labels[:,1:].sum(-1)>0, 1, 0)
            loss = self.loss(logits.float(), labels_ident.float())
            output["loss"] = {"prune_loss": loss.to(sequence_output)}
