    ...
    def finetune(features, optimizer, num_epoch, num_steps):
        ...
        for epoch in tqdm(train_iterator, desc='Train epoch'):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                outputs = model(inputs['input_ids'], inputs['attention_mask'], inputs['labels'], inputs['entity_pos'], inputs['hts'], inputs['tag'])
                loss = [outputs["loss"]["prune_loss"]]
                ...
                if (step + 1) == len(train_dataloader) or (
                    ...
                    dev_scores, dev_output = evaluate(args, model, dev_features, tag="dev")
                    wandb.log(dev_scores, step=num_steps)
                    print(dev_scores)

                    if dev_scores["f1"] > best_score:
                        best_score = dev_scores["f1"]
                        best_all_score = dev_scores
...
def evaluate(args, model, features, tag="dev"):
    dataloader = DataLoader(features, batch_size=args.test_batch_size, shuffle=False, collate_fn=collate_fn,
                            drop_last=False)
    preds = []
    total_pred, acc_pred, total_gold = 0, 0, 0
    ...
    for batch in tqdm(dataloader, desc=f"Evaluating batches"):
        ...
        with torch.no_grad():
            ...
            ident_statistic = outputs["ident_statistic"]
            total_pred += ident_statistic["preds_num"]
            acc_pred += ident_statistic["correct_num"]
            total_gold += ident_statistic["gold_num"]

            rel_pred = outputs["rel_pred"]
            preds.extend(rel_pred)

    precision = acc_pred/(total_pred+1e-9)
    recall = acc_pred/(total_gold+1e-9)
...
# def dump_to_file(pred_file, best_output, score_file, dev_scores):
#     '''
...
def main():
    ...
    model = Pruner(config, model, tokenizer,
